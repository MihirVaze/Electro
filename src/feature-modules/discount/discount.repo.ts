...
class DiscountRepo {
  ...
  public async update(user: Partial<Discount>, options: UpdateOptions<Discount>, schema: string) {
    return DiscountSchema.schema(schema).update(user, options);
  };

  public async delete(options: UpdateOptions<Discount>, schema: string) {
    return DiscountSchema.schema(schema).update({ isDeleted: true }, options);
  };
};
