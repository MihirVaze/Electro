...
class DiscountServices {
  ...
  async getdiscounts(limit: number, page: number, discount: Partial<Discount>, schema: string) {
    try {
  ...
  async updateDiscount(id: string, discount: Partial<Discount>, schema: string) {

    try {
      if (!discount.id) throw "ID NOT FOUND"
      const result = await discountRepo.update(discount, { where: { id: discount.id } }, schema);
