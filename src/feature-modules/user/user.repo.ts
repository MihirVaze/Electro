...
class UserRepo {
    public async createUser(user: User, schema: string) {
        return UserSchema.schema(schema).create(user);
    }

    public async getUser(options: FindOptions<User>, schema: string) {
        return UserSchema.schema(schema).findOne(options);
    }

    public async getAllUser(options: FindOptions<User>, schema: string) {
    ...
    public async deleteUser(options: UpdateOptions<User>, schema: string) {
        return UserSchema.schema(schema).update({ isDeleted: true }, options);
    }

    public async createUserRole(UserRole: UserRole, schema: string) {
        return UserRoleSchema.schema(schema).create(UserRole);
    }

    public async getUserRole(options: FindOptions<UserRole>, schema: string) {
