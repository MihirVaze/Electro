...
router.post('/:id',[ upload.single('avatar'),validate(uiData),async (req,res,next)=>{
    try{
        if(!req.file){
    throw {status:400, message:"Bad Request Logo Required"}
     ...
     const result= await clientUIService.createClientUI(details,req.params.id);
...
router.patch('/:id',[ async (req,res,next)=>{
    try{
        const result= await clientUIService.updateUIDetails(req.body,req.params.id);
        res.send(new ResponseHandler(result));
    }
    catch(e){
        next(e);
